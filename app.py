...
# # Define the classes based on your requirement
...
# # Define a function to classify the device
# def classify_device(image_rgb):
#     # Function to preprocess image as required by your model
#     def preprocess_image(img):
#         img = cv2.resize(img, (299, 299))  # Resize image as per your model's input size
#         img = np.expand_dims(img, axis=0)  # Add batch dimension
#         img = preprocess_input(img)  # Preprocess image as required by the model
#         return img
...
#     glucose_values = []
#     device_type = classify_device(image_rgb)
...
#                 st.write(f"- {value['Image']} | {value['Glucose Value']} | {value['Device Type']}")
#         else:
...
#             all_values_df = pd.read_csv(file_path)
#             all_values_df = pd.concat([all_values_df, new_df], ignore_index=True)
#         else:
#             all_values_df = new_df.copy()

#         all_values_df.drop_duplicates(inplace=True)
#         all_values_df.to_csv(file_path, index=False)
...
# all_device_values = pd.DataFrame(columns=['Image', 'Glucose Value'] + classes)
